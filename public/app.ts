...
var endpoint =
  "https://mobility.api.opendatahub.bz.it/v2/flat/ParkingStation,ParkingSensor/";
...
parking.run(function ($rootScope) {
  ...
  if (!$rootScope.i18n[$rootScope.lang]) lang = "en";
...
parking.controller(
  "parking",
  function ($scope, $http, $interval, $window, leafletData, $filter, $q) {
    var self = $scope;
    ...
    self.conditionalSorting = function (obj) {
      if (
        ...
        if (obj.current) return obj.current.timestamp * -1;
        else return 0;
      }
    };
    function drawCharts() {
      ...
      for (stationId in self.predictions) {
        var data = [];
        for (const i in self.predictions[stationId]) {
          var point = {
        ...
        if (!self.chartData) self.chartData = {};
    ...
    self.init = function () {
      ...
      ); //first parameter success callback, second fail callback
      angular.extend(self, {
        southTyrol: {
          lat: 46.629849,
          lng: 11.3711693,
    ...
    let convertSensorsToStations = function (data) {
      const parkingStations = data
        .filter((s) => s.stype == "ParkingSensor")
        .reduce((stations, sensor) => {
    ...
    self.updateAllPopUps = function (stations, feature, layer) {
      if (stations) {
        stations.forEach(function (station, index) {
          if (
            station.scode == feature.properties.stationcode &&
            station.mvalue != undefined
          ) {
            var free_places =
              station.stype === "ParkingSensor"
                ? station.mvalue * -1 + 1
      ...
      angular.extend(self.geojson, {
        parking: {
          ...
          pointToLayer: function (feature, latlng) {
            ...
            if (current < 90) image = "marker-icon-green.png";
            else if (current >= 90 && current < 100) {
              image = "marker-icon-yellow.png";
            } else if (current == 100) image = "marker-icon-red.png";
