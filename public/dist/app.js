...
parking.controller('parking', function ($scope, $http, $interval, $window, leafletData, $filter, $q) {
    ...
    self.conditionalSorting = function (obj) {
        if (self.currentPosition && obj.scoordinate.y && obj.scoordinate.x) {
            var distance = geolib.getDistance({ longitude: self.currentPosition.coords.longitude, latitude: self.currentPosition.coords.latitude }, { longitude: obj.scoordinate.x, latitude: obj.scoordinate.y });
            return distance;
        }
        else {
            if (obj.current)
    ...
    self.getData = function (callback) {
        $http.get(endpoint + 'occupied/latest?limit=-1&offset=0&shownull=false&distinct=true').then(function (response) {
            if (response.status == 200) {
                ...
                if (Object.entries(self.mMap).length === 0) {
                    var distinctMunicipalities = __spreadArrays(new Set(data.map(function (item) { return item.smetadata.municipality; }))).sort();
                    var indexOf = void 0;
                    self.mMap = distinctMunicipalities.reduce(function (map, item) { return (map[item] = { active: item && item.indexOf('Bozen') != -1, value: item }, map); }, {});
                }
                drawGJ();
        ...
        convertSensorsToStations = function (data) {
            parkingStations = data.filter(function (s) { return s.stype == 'ParkingSensor'; }).reduce(function (stations, sensor) {
